...
#ifndef CONFIG_JZ_SPI_SELECT_EXT_CLK
...
void jz_spi_clk_rate(struct jz_spi *hw, unsigned long *rate) {
#ifndef CONFIG_JZ_SPI_SELECT_EXT_CLK
...
#ifndef CONFIG_JZ_SPI_SELECT_EXT_CLK
#ifdef CONFIG_JZ_SFC
	if((2 * hz) > clk_get_rate(hw->clk)) {
		pr_err("The max_speed_hz can not more than sfc clock\n");
		return -EINVAL;
	}
#else
...
#else
	if(hz > JZ_EXTAL) {
		pr_err("The max_speed_hz can not more than ext_clk(%d)\n", JZ_EXTAL);
		return -EINVAL;
	}
#endif

	jz_spi_clk_rate(hw, &cpm_rate);
	if(cpm_rate < hz) {
		pr_err("ssi set clk rate err!\n");
		return -EINVAL;
	}
...
static int __init jz_spi_probe(struct platform_device *pdev)
{
	...
	/*  set clk source, select sfc_clk or ext_clk */
	clk_set_parent(hw->clk, clk_get_parent(hw->clk));
