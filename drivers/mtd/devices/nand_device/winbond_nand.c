...
static void active_die(uint8_t die_id) {
	...
	if(sfc_sync(winbond_flash->sfc, &message)) {
	        dev_err(winbond_flash->dev,"sfc_sync error ! %s %s %d\n",__FILE__,__func__,__LINE__);
		return;
	}
}
#ifdef WINBOND_DEBUG
static void winbond_reset(void) {
...
static void winbond_write_enable(struct sfc_transfer *transfer, struct cmd_info *cmd, uint8_t device_id, uint32_t pageaddr) {
