...
static int32_t jz_sfc_nand_read(struct sfc_flash *flash, int32_t pageaddr, int32_t columnaddr, char *buffer, size_t len)
{
	...
	if(nand_desc->param.need_quad) {
		if(nand_read_ops->quad_read) {
		     ...
		     *
		     * */
			nand_quad_read(&transfer, &cmd, columnaddr, 2, buffer, len);
		}
	} else {
...
static int32_t jz_sfc_nand_write(struct sfc_flash *flash, u_char *buffer, uint32_t pageaddr, uint32_t columnaddr, size_t len)
{
	...
	if(nand_write_ops->write_enable)
		nand_write_ops->write_enable(transfer, cmd, nand_desc->id_device, pageaddr);
	else
...
static int jz_sfcnand_write_oob(struct mtd_info *mtd, loff_t addr, struct mtd_oob_ops *ops)
{
	...
	if((ret = jz_sfc_nand_write(flash, flash->swap_buf, oob_addr / mtd->writesize, mtd->writesize, ops->ooblen))) {
...
static int32_t jz_sfc_nand_erase_blk(struct sfc_flash *flash, uint32_t pageaddr)
{
	...
	if(nand_erase_ops->write_enable)
		nand_erase_ops->write_enable(transfer, cmd, nand_desc->id_device, pageaddr);
	else
		nand_write_enable(transfer, cmd);
	sfc_message_add_tail(transfer, &message);
...
static int jz_sfcnand_write(struct mtd_info *mtd, loff_t to, size_t len, size_t *retlen, const u_char *buf)
{
	...
	while(len) {
		...
		memcpy(flash->swap_buf, buf, wlen);

		if((ret = jz_sfc_nand_write(flash, flash->swap_buf, pageaddr, columnaddr, wlen))) {
...
static int32_t __init nand_partition_param_copy(struct sfc_flash *flash, struct jz_sfc_nand_burner_param *burn_param) {
	struct jz_nand_descriptor *nand_desc = flash->flash_info;
	int i = 0, count = 5, ret;
	size_t retlen = 0;
...
static int32_t __init jz_nand_partition(struct sfc_flash *flash) {
